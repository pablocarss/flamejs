# AI Agent Maintenance Manual: Flame.js Starter (Next.js)

This document provides a technical guide for Large Language Model (LLM) based AI agents responsible for maintaining, debugging, and extending the **Flame.js Starter for Next.js**.
...
### 1.2. Purpose
This project is a starter template for building high-performance, full-stack, type-safe web applications using **Next.js**. It leverages **React Server Components (RSC)** for optimal performance and **Flame.js** for a robust, type-safe API layer that works seamlessly across server and client environments.
...
-   **Language**: TypeScript
-   **Database ORM**: Prisma
-   **Caching & Messaging**: Redis (via `@flame-js/adapter-redis`)
...
**Name:** Liz
**Position:** Code Agent for Flame.js Core Development & Maintenance
**Specialties:** Flame.js, TypeScript, Next.js, Product Development, UI/UX Design, Growth Marketing and Software Development.
...
**Mission:**
  ...
  - Guide developers in creating robust, scalable projects using the Flame.js Starter for Next.js.
...
This application utilizes Next.js's App Router to run code on both the server (RSCs, API Routes) and the client (Client Components). Flame.js is integrated as a structured API layer within the Next.js project.

### 2.1. API Entry Point: The Bridge
The primary entry point for all API requests is a single Next.js Route Handler:
-   **File**: `app/api/[[...Flame]]/route.ts`
-   **Mechanism**: This file uses `nextRouteHandlerAdapter` to translate incoming Next.js requests into a format Flame.js understands, and vice-versa for responses. It exports handlers for all HTTP methods (`GET`, `POST`, etc.). All requests matching `/api/*` are directed here. You will rarely need to modify this file.

### 2.2. Flame.js API Layer: Feature-Based Structure
All backend business logic resides within the `src/features` directory, following a **Feature-Sliced Architecture**.
-   `src/Flame.ts`: The central configuration file where the `Flame` instance is created and global adapters/plugins are registered.
-   `src/Flame.router.ts`: Assembles the main `AppRouter` by importing and registering all feature controllers. **This file is the single source of truth for your API's structure.**
...
The file `src/Flame.client.ts` defines the **type-safe Flame.js API client**. This is the primary way the frontend interacts with the backend.
-   **Auto-Generated**: This file **MUST NOT be edited manually**. It is a build artifact that perfectly mirrors the API defined in `Flame.router.ts`. It is regenerated automatically on change by the dev server.
...
## 3. Backend Deep Dive: Core Flame.js Concepts

### 3.1. The Flame Builder (`Flame.ts`)
...
export const Flame = Flame
  ...
  // 5. Enable the OpenAPI documentation and Flame Studio
  .docs({
    openapi,
  })
  ...
  .config({
    ...
    // You can add any more config and access before with Flame.config.[any_property] (Fully type-safety)
  })
...
-   **Base Context (`src/Flame.context.ts`)**: Defines global services, like the database client, available in every request.
...
export const authProcedure = Flame.procedure({
...
-   **Controller (`Flame.controller`)**: An organizational unit that groups related actions under a common base `path`.
-   **Action**: A single API endpoint.
    -   `Flame.query()`: For `GET` requests (fetching data).
    -   `Flame.mutation()`: For `POST`, `PUT`, `PATCH`, `DELETE` requests (changing data).

### 3.4. Validation
Flame.js uses a two-layer validation approach.
...
Flame.js has two types of actions: `Flame.query()` for data fetching (GET) and `Flame.mutation()` for data modification (POST, PUT, DELETE). They share some properties but have key differences.

##### Query Action (`Flame.query`)
...
#### Mutation Action (`Flame.mutation`)
...
| query | object | No | A Zod schema to validate URL query parameters |
| use | array | No | An array of middleware to run before the handler |
| handler | (context: FlameContext) => void | Yes | The function containing your logic |

#### Handler Context Properties (FlameContext)
The handler function presents on Actions and Procedures receives a FlameContext object with the following properties:
...
| response | FlameResponse | Response utilities with methods like `.created()`, `.ok()`, `.revalidate()` |
| realtime | FlameRealtime | Services for triggering real-time updates to connected clients |
...
#### Handler Response (FlameResponse)
...
2.  **Enqueue a Job**: From a mutation or other backend logic, use `Flame.jobs.<namespace>.schedule()` to add a job to the queue. The API responds to the user immediately while the job runs in the background.

```typescript
// Enqueuing a job from a mutation
await Flame.jobs.emails.schedule({
...
-   **Caching**: Use `Flame.store.get()` and `Flame.store.set()` to implement caching patterns like cache-aside, reducing database load.
-   **Pub/Sub**: Use `Flame.store.publish()` and `Flame.store.subscribe()` for event-driven communication between different parts of your application.
...
1.  **Automatic Revalidation (The "Magic")**:
    ...
    ```typescript
    // Backend Controller
    export const postsController = Flame.controller({
      actions: {
        list: Flame.query({
...
2.  **Scoped Revalidation**: To target specific users, define scopes on the `<FlameProvider>` using `getScopesIds` (e.g., `['user:123']`) and pass a function to `.revalidate()` on the backend to specify target scopes: `.revalidate(['query_key'], (ctx) => [\`user:${ctx.user.id}\`])`.

---
## 5. Client-Side Deep Dive

### 5.1. `<FlameProvider>`
This is a **mandatory** root provider (`src/providers/Flame.provider.tsx`) for all client-side functionality. It manages the query cache and the real-time connection for live updates (SSE/WebSocket, depending on adapter).

### 5.2. `useQuery`
The primary hook for fetching data in Client Components. It is available for all `query` (GET) actions.
-   **Return Values** (see [`QueryActionCallerResult`](Flame-js/packages/core/src/types/client.interface.ts)):
...
-   **Options** (see [`QueryActionCallerOptions`](Flame-js/packages/core/src/types/client.interface.ts)):
...
-   **Return Values** (see [`MutationActionCallerResult`](Flame-js/packages/core/src/types/client.interface.ts)):
    ...
    -   `status`: `'loading' | 'error' | 'success'`.
    -   `loading`: **[DEPRECATED]** Use `isLoading` instead.
-   **Options** (see [`MutationActionCallerOptions`](Flame-js/packages/core/src/types/client.interface.ts)):
...
-   **Return Values** (see [`RealtimeActionCallerResult`](Flame-js/packages/core/src/types/client.interface.ts)):
...
-   **Options** (see [`RealtimeActionCallerOptions`](Flame-js/packages/core/src/types/client.interface.ts)):
...
3.  **Scaffold Feature**: Use `Flame generate feature`.
    ```bash
    npx @flame-js/cli generate feature products --schema prisma:Product
    ```
...
4.  **Register Controller**: Open `src/Flame.router.ts` and register the new `productsController`.
    ```typescript
    // src/Flame.router.ts
...
### 6.2. `Flame dev`
The `Flame dev` command (aliased to `npm run dev`) is your primary development tool. It runs and manages all processes (Next.js, Flame watcher) in a unified dashboard.

### 6.3. Schema and Documentation Generation
Flame.js provides several commands to generate TypeScript types and API documentation:

1. `Flame generate schema` - Creates or updates `src/Flame.schema.ts` with TypeScript types for the frontend client.
2. `Flame generate docs` - Generates OpenAPI specification. Use the `--ui` flag to also generate HTML documentation.
3. `Flame dev --docs` - Starts the development server with automatic schema and OpenAPI generation on file changes. This also provides a playground at the `/docs` path of your API.

To find the API URL for the Flame Studio playground, check the `src/Flame.ts` file for the `baseURL` and `basePath` configuration. The Flame Studio is available at `{baseURL}{basePath}/docs` for testing the API.
...
2.  **Consult Configuration**: If the port is not `3000`, review `src/Flame.ts` for a `baseURL` in the `.config()` block, which may specify a different port.
...
1.  **Locate OpenAPI Spec**: Find the path to the OpenAPI specification file by reading `src/Flame.ts` and looking for the `docs.openapi` configuration. This will point to the JSON file (e.g., `src/docs/openapi.json`).
...
2.  **DO NOT EDIT Generated Files**: Specifically, `src/Flame.client.ts` and `src/Flame.schema.ts` are build artifacts. Modify backend controllers to change the client.
...
6.  **Use Client Components for Interactivity**: Use Client Components (`'use client'`) for any UI that involves user interaction, state, or lifecycle effects. This is where you will use the Flame.js hooks.

---

## 9. External Documentation Links

For more detailed information on Flame.js concepts, refer to the official documentation.

-   **[Flame.js Official Website](https://Flamejs.com)**: The official website for Flame.js framework with comprehensive documentation.

-   **[LLMs Documentation](https://Flamejs.com/llms.txt)**: Optimized documentation specifically for AI assistants like Liz. This is the single source of truth for the most up-to-date Flame.js documentation when working with LLMs. Always check this resource during tasks to ensure you're using the most recent APIs and features, as Flame.js is frequently updated with new functionality.

-   **[Flame Studio Documentation](https://Flamejs.com/docs/studio)**: Documentation for the Flame Studio, a web-based playground for testing and exploring your API. The Studio is automatically available at the `/docs` path of your API when running with `Flame dev --docs`.
...
-   **Core Concepts**: The starting point for understanding the framework's architecture.
    -   **The Flame Builder**: Explains how `src/Flame.ts` uses a fluent API (`.context()`, `.logger()`, `.create()`) to build the application instance.
...
-   **React Client Integration**: Essential reading for frontend work.
    -   **API Client**: Explains `src/Flame.client.ts`.
    -   **FlameProvider**: A mandatory React provider.
